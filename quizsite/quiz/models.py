    ...
    def _validation_status_field(self):
        ...
        if self.status == Status.PUBLISHED:
            ...
            if amount != Questions.false_answers_count:
                raise ValidationError(f"Неправильное количество связанных с вопросом ответов."
                                      f"\nДолжно быть {Questions.false_answers_count} неправильных ответов."
                                      f"\nТекущее количество неправильных ответов: {amount}")
            amount = len(connect_answers.filter(correct_or_wrong=Answers.CorrectOrWrong.CORRECT))
            if amount != Questions.true_answers_count:
                raise ValidationError(f"Неправильное количество связанных с вопросом ответов."
                                      f"\nДолжно быть {Questions.true_answers_count} правильных ответов."
                                      f"\nТекущее количество правильных ответов: {amount}")
